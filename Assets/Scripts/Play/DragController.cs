...
public class DragController : HitObject
{
    ...
    private void Judge()
    {
        ...
        {
            if (HitJudge.Record)
                HitJudge.RecordLog.AppendLine("[AutoMiss-TooLate] " + Index + "(Drag) Missed <At " + From + ">");
            HitJudge.JudgeMiss(this);
            missed = true;
            Line.RemainingNote--;
            gameObject.SetActive(false);
        }
        if (!missed && !Hit && From - AudioUpdate.Time <= GameSettings.Perfect2 && AudioUpdate.Time - From <= GameSettings.Bad)
        {
            bool ok = false;
